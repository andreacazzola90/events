...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log(
      "[API /events POST] Request received, Content-Type:",
      contentType,
    );
    console.log(
      "[API /events POST] Prisma Client version check: category is now a String",
    );
    if (contentType.includes("application/json")) {
      // Handle JSON body
      eventData = await request.json();
      console.log(
        "[API /events POST] JSON body received:",
      ...
      if (imageUrl && !imageUrl.includes("supabase.co")) {
        try {
          console.log(
            "[API /events POST] Downloading external image:",
            imageUrl,
          );
          ...
          console.log(
            "[API /events POST] Uploading external image to Supabase...",
          );
          imageUrl = await uploadImageToSupabase(imageBuffer, "events");
          console.log(
            "[API /events POST] External image uploaded to Supabase:",
            imageUrl,
          );
        } catch (uploadError) {
          console.warn(
            "[API /events POST] Failed to upload external image, keeping original URL:",
            uploadError,
          );
    ...
    if (eventDataToSave.location) {
      try {
        ...
        console.log(
          "[API /events POST] Geocoded location to coordinates:",
          coords,
        );
      } catch (geoError) {
        console.warn(
          "[API /events POST] Failed to geocode location, continuing without coordinates:",
          geoError,
        );
      }
    }
    ...
    console.log(
      "[API /events POST] Saving event with data:",
      JSON.stringify(eventDataToSave, null, 2),
    );
    ...
    console.log(
      "[API /events POST] Cache revalidated for home, eventi, mappa pages and events-list tag",
    );

    try {
      const listingUrl = new URL(
        `/api/events?limit=200&_rebuild=${Date.now()}`,
        request.nextUrl.origin,
      ).toString();
      await fetch(listingUrl, { cache: "no-store" });
      console.log(
        "[API /events POST] Events listing JSON rebuilt:",
        listingUrl,
      );
    } catch (listingRebuildError) {
      console.warn(
        "[API /events POST] Events listing JSON rebuild failed, continuing:",
        listingRebuildError,
      );
    }
    ...
    try {
      ...
      console.warn(
        "[API /events POST] Event detail pre-warm failed, continuing:",
        prewarmError,
      );
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!userIdParam) {
      ...
      responseEvents = events.filter((event: DbEvent) => {
